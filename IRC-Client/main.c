...
#define NICKNAME "SKLDoksjdwk<SKkdKSLDKSmcskds"          
...
char *format_command(const char *input) {
    ...
    char *formatted = (char*)malloc(BUFFER_LEN * sizeof(char));
    if (!formatted) {
    ...
    if(checked == false){
        printf("\n|ERROR| - Invalid format. Try using / before the command\n\n");
        return " |ERROR|";
    } 
    return formatted;
}
...
void send_message(SOCKET sockfd, const char *message) {
    ...
    strcpy(formatted, format_command(message));
...
char* menu() {
    int choice;
    char *username = (char*)malloc(MAX_USERNAME_LEN * sizeof(char)); // dynamically allocate memory
    if (!username) {
    ...
    while (1) {
        ...
        switch (choice) {
            case 0:
                ...
                sleep(2);
                printf("\nGoodbye!\n");
                free(username);  // Don't forget to free memory
                exit(0);
            case 1:
